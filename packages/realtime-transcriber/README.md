...
BlackHole 2ch 経由でキャプチャした音声を MLX-Whisper（`mlx-community/whisper-large-v3-turbo`）で文字起こしし、AWS Translate で日本語に翻訳して表示します。60秒ごとに Amazon Bedrock（Claude Haiku 4.5）で内容を要約し、Whisper の認識精度向上にも活用します。
...
- MLX-Whisper モデル（`whisper-large-v3-turbo`）: 約 1.5 GB（初回起動時に `~/.cache/huggingface/hub/` へ自動ダウンロード）
...
- `⏳ Translating...` — AWS Translate が翻訳中
...
- $15.00 / 100万文字
- 1時間のセッションで約 5,000〜15,000 文字を翻訳すると仮定すると、**約 $0.08〜$0.23 / 時間**

### Amazon Bedrock（Claude Haiku 4.5）

- 入力: $0.80 / 100万トークン
- 出力: $4.00 / 100万トークン
- 60秒ごとに1回呼び出し。1回あたり入力 ~600 トークン・出力 ~200 トークンと仮定すると、1時間（60回）で約 **$0.09 / 時間**

### 合計目安

1時間のセッションで概ね **$0.20 前後**（内容の密度によって変動します）。
